...
pow_tbl_headers = {'subject','predictability','movement','area','power','powerBlCorr'};
pow_tbl = cell2table(cell(0,length(pow_tbl_headers)),'VariableNames', pow_tbl_headers);
coh_tbl_headers = {'subject','predictability','movement','area','coh','cohBlCorr'};
    ...
    for j = 1:numel(conditions)
        ...
        for h = 1:numel(events)
            ...
            for s = 1:length(hemispheres)
                ...
                if gamma == 1
                    save([save_here,subj,'_',current_condi,'_',this_event,'_',hemi,'_','powandcoh_gamma.mat'],'tfr','spec','band','tfr_blcorr','spec_blcorr','band_blcorr','band_time_blcorr','cross_corr');
                else
                    save([save_here,subj,'_',current_condi,'_',this_event,'_',hemi,'_','powandcoh.mat'],'tfr','spec','band','tfr_blcorr','spec_blcorr','band_blcorr','band_time_blcorr','cross_corr');
                end
                ...
                this_pow_tbl = table(repmat({subj},[length(band_pow), 1]),repmat({current_condi},[length(band_pow), 1]),...
                    repmat({this_event},[length(band_pow),1]),band_pow_chans, band_pow, band_pow_blcorr,...
                    'VariableNames',{'subject','predictability','movement','area','power','powerBlCorr'});
                ...
                this_coh_tbl = table(repmat({subj},[length(band_coh), 1]),repmat({current_condi},[length(band_coh), 1]),...
                    repmat({this_event},[length(band_coh),1]),band_coh_chans, band_coh, band_coh_blcorr,...
                    'VariableNames',{'subject','predictability','movement','area','coh','cohBlCorr'});
...
%to save tables for ANOVA
if gamma == 1
    save([save_here,'tables_for_stats_',(alignedto),'_gamma.mat'],'pow_tbl','coh_tbl');
else
    save([save_here,'tables_for_stats_',(alignedto),'_final.mat'],'pow_tbl','coh_tbl');
end
